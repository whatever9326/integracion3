  ...
  document.getElementById('generar').addEventListener('click', async () => {
    ...
    reader.onload = async (e) => {
      ...
      rows.forEach((row, index) => {
        ...
        const mappedModel = modelMap[currentModel] || currentModel;
        const fullModel = `${pedido}-${mappedModel}`;
        ...
        colors.forEach((color, idx) => {
          ...
          const foundSku = skus.find(sku => sku.includes(fullModel) && sku.includes(finalColor));

          if (foundSku) {
            console.log(`✅ Encontrado SKU: ${foundSku} → ${qty}`);
            counts[foundSku] = (counts[foundSku] || 0) + Number(qty);
          } else {
            console.warn(`❌ No se encontró SKU para ${fullModel} + ${finalColor}`);
          }
        });
      });
